...
const InsightsGrp = () => {
  ...
  useEffect(() => {
    const fetchHealthPoints = async () => {
      try {
        ...
        if (!response.success) {
          toast({ title: 'Error', description: 'Could not fetch health group data.' });
        } else {
          setGrpData(response.healthGrpData || []);
        }
      } catch (error) {
        console.error('Error fetching group data:', error);
        toast({ title: 'Error', description: 'An error occurred while fetching group data.' });
      }
    };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Health Insights
        </CardTitle>
        <CardDescription>Your health progress over time
        </CardDescription>
        {grpData.length < 2 && <CardDescription className=' text-green-600'>Check-In daily to get your graph more elobrated</CardDescription>}
      </CardHeader>
      <CardContent>
        {grpData.length === 0 ? (
          <p className="text-center dark:text-muted-foreground">No Data Found</p>
        ) : (
